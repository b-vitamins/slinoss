...
def derive_nextchar_budget(sample: dict[str, float]) -> dict[str, float]:
    ...
    out["forward.mixer.bc_emit"] = sample.get("forward.mixer.bc_emit", 0.0)
    ...
    out["forward.mixer.tail"] = sample.get(
        "forward.mixer.tail",
        _sum("forward.mixer.gate", "forward.mixer.out_proj"),
    )
    ...
    out["backward.mixer.bc_emit"] = sample.get("backward.mixer.bc_emit", 0.0)
    ...
    out["backward.mixer.tail"] = sample.get(
        "backward.mixer.tail",
        _sum("backward.mixer.gate", "backward.mixer.out_proj"),
    )
